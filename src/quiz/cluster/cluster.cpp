...
void proximity(int it, const std::vector<std::vector<float>>& point, std::vector<int>& cluster_p, std::vector<bool> &processed, KdTree* tree, float distanceTol)
{
	processed[it] = true;
	cluster_p.push_back(it); 

	std::vector<int> nearest = tree->search(point[it], distanceTol);

	for(int id : nearest)
	{
		// if the points nearest to the points[it] are not processed add the points in cluster
		if(processed[id] == false)
			proximity(id, point, cluster_p, processed, tree, distanceTol);
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	while (i<points.size())
	{
		// if the points are already added to the clusters list by using tree search method
		if(processed[i]==true)
		{
			i++;
		}

		// else some points are not in the cluster detected so, search for more
...
int main ()
{
  	...
  	for(int index=0; index < nearby.size(); index++)
	{
  	...
  	//
  	std::vector<std::vector<int>> clusters = euclideanCluster(points, tree, 3);
  	//
